...
static void tas2559_hw_reset(struct tas2559_priv *pTAS2559)
{
	...
	if (gpio_is_valid(pTAS2559->mnDevAGPIORST)) {
		//gpio_set_value(pTAS2559->mnDevAGPIORST,1);
		//msleep(5);
		//gpio_direction_output(pTAS2559->mnDevAGPIORST, 0);
		gpio_set_value(pTAS2559->mnDevAGPIORST,0);
		msleep(5);
			     ...
			     const struct i2c_device_id *pID)
{
	...
	if (gpio_is_valid(pTAS2559->mnDevAGPIORST)) {
		...
		nResult = devm_gpio_request_one(&pClient->dev, pTAS2559->mnDevAGPIORST,
					    GPIOF_DIR_OUT, "tas2559 reset");

		/*dev_err(pTAS2559->dev, "%s: GPIO %d request error\n",
				__func__, pTAS2559->mnDevAGPIORST);*/

		if (nResult < 0) {
			dev_err(pTAS2559->dev, "%s: GPIO222 %d request error\n",
				__func__, pTAS2559->mnDevAGPIORST);
			goto err;
		}
	}
	...
	pTAS2559->mnLeftEdge = 4;
	pTAS2559->mnRightEdge = 4;
	//pTAS2559->mnCurrentSampleRate = 48000;//defalut 48Khz

	mutex_init(&pTAS2559->dev_lock);

	/* Reset the chip */
	nResult = tas2559_dev_write(pTAS2559, DevA, TAS2559_SW_RESET_REG, 1);
	if (nResult < 0) {
		dev_err(&pClient->dev, "I2c fail, %d\n", nResult);
		//goto err;
	}
	msleep(100);
	dev_info(&pClient->dev, "tas2559_dev_write= %d\n", nResult);
