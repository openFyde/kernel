...
static unsigned int p_tas2559_startup_data[] = {
	...
	DevA, TAS2559_UDELAY, 2000,/*delay*/
	//DevB, TAS2560_MUTE_REG, 0x41,
	//DevB, TAS2559_UDELAY, 2000,/*delay*/
...
static unsigned int p_tas2559_shutdown_data[] = {
	...
	DevA, TAS2559_MDELAY, 20,/*delay 20ms 11*/
	DevA, TAS2559_MUTE_REG, 0x03,/*mute*/
			       ...
			       unsigned int dev)
{
	...
	dev_dbg(pTAS2559->dev, "%s, dev11111=  %d\n", __func__, dev);
...
int tas2559_get_die_temperature(struct tas2559_priv *pTAS2559, int *pTemperature)
{
	...
	if (!pTAS2559->mbPowerUp) {
		dev_dbg(pTAS2559->dev, "%s, device not powered on\n", __func__);
		goto end;
	}
...
void tas2559_fw_ready(const struct firmware *pFW, void *pContext)
{
	...
	pTAS2559->read(pTAS2559, DevA, TAS2559_REG(0, 1, 0x3d), &value);
	pTAS2559->read(pTAS2559, DevA, TAS2559_REG(0, 1, 0x3e), &value1);
	pTAS2559->read(pTAS2559, DevA, TAS2559_REG(0, 1, 0x3f), &value2);

    pTAS2559->write(pTAS2559, DevA, TAS2559_REG(0, 0, 0x06), 0x64);//change 15db to 12db
    //pTAS2559->write(pTAS2559, DevA, TAS2559_REG(0, 0, 0x06), 0x4C);//change 12db to 9db
	pTAS2559->write(pTAS2559, DevA, TAS2559_REG(0, 0, 0x22), 0x22);
	mdelay(100);
	//tas2559_set_sampling_rate(pTAS2559,48000);
	tas2559_enable(pTAS2559,1);//temp solution for pop	
			...
			unsigned int nProgram, int nConfig)
{
	...
	if (nConfig < 0) {
		nConfiguration = 0;
		nSampleRate = pTAS2559->mnCurrentSampleRate;
		dev_info(pTAS2559->dev, "pTAS2559->mpFirmware->mnConfigurations %d\n", pTAS2559->mpFirmware->mnConfigurations);
		dev_info(pTAS2559->dev, "samplerate %d === %d\n", pTAS2559->mpFirmware->mpConfigurations[0].mnSamplingRate,pTAS2559->mpFirmware->mpConfigurations[1].mnSamplingRate);
	...
	pTAS2559->hw_reset(pTAS2559);
	nResult = pTAS2559->write(pTAS2559, DevA, TAS2559_SW_RESET_REG, 0x01);
	//if (nResult < 0)
		//goto end;
...
int tas2559_set_calibration(struct tas2559_priv *pTAS2559, int nCalibration)
{
	...
	/*if ((!pTAS2559->mpFirmware->mpPrograms)
	    || (!pTAS2559->mpFirmware->mpConfigurations)) {
		dev_dbg(pTAS2559->dev, "Firmware not loaded\n\r");
		nResult = 0;
		goto end;
	}
...
int tas2559_parse_dt(struct device *dev, struct tas2559_priv *pTAS2559)
{
	...
	if (!gpio_is_valid(pTAS2559->mnDevAGPIORST))
		dev_dbg(pTAS2559->dev, "Looking up %s property in node %s failed %d\n",
