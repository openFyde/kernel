...
//add by rpdzkj for touch Goodix TP wakeup lcd	20181219
/*******************************************************
Function:
    Computing kernel time function
Input:
    None
Output:
    ts:kernel time
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
    ...
    input_set_abs_params(ts->pen_dev, ABS_MT_PRESSURE, 0, 30, 0, 0);
    input_set_abs_params(ts->pen_dev, ABS_MT_TOUCH_MAJOR, 0, 30, 0, 0);
...
//#if GTP_CONFIG_OF	
    ...
    u8 temp_val[2]={0,0};
    //ret = gtp_i2c_read(ts->client, temp_val, GTP_ADDR_LENGTH);
    ret = gtp_i2c_read_dbl_check(ts->client, 0x814E, &temp_val[0], 2);
    if (FAIL == ret){
		GTP_INFO("gtp_i2c_read 0x814E error\n");
    }else{
		GTP_INFO("gtp_i2c_read 0x814E value = 0x%X 0x%X\n",temp_val[0],temp_val[1]);
    }
    
    u8 buf[4] = {(0x814E ) >> 8, (0x814E) & 0xff};
    
    ret = gtp_i2c_read(ts->client, buf, 2 ); 
    if (FAIL == ret){
		GTP_INFO("gtp_i2c_read 2 0x814E error");
    }else{
		GTP_INFO("gtp_i2c_read 2 0x814E value = 0x%X 0x%X",buf[2],buf[3]);
    }
...
static s8 gtp_i2c_test_my(struct i2c_client *client)
{
    u8 test[4] = {0x8047 >> 8, 0x8047 & 0xff};
    u8 retry = 0;
    s8 ret = -1;
    ...
    while(retry++ < 5)
    {
        ...
        if (ret > 0)
        {
        	GTP_INFO("GTP i2c test_my read 0x%X  0x%X 0x%X  0x%X.",test[0],test[1],test[2],test[3]);
            return ret;
        }
        GTP_ERROR("GTP i2c test_my failed time %d.",retry);
        msleep(10);
    }
    return ret;
}
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    input_abs_set_res(ts->input_dev, ABS_MT_POSITION_X, gtp_config->gtp_resolution_x);
    input_abs_set_res(ts->input_dev, ABS_MT_POSITION_Y, gtp_config->gtp_resolution_y);
    input_set_abs_params(ts->input_dev, ABS_X, 0, ts->abs_x_max, 0, 0);
    input_abs_set_res(ts->input_dev, ABS_X, gtp_config->gtp_resolution_x);
    input_set_abs_params(ts->input_dev, ABS_Y, 0, ts->abs_y_max, 0, 0);
    input_abs_set_res(ts->input_dev, ABS_Y, gtp_config->gtp_resolution_y);
#if GTP_ICS_SLOT_REPORT
    input_set_abs_params(ts->input_dev, ABS_MT_TOOL_TYPE, MT_TOOL_FINGER, MT_TOOL_PALM, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_PRESSURE, 0, 100, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE, 0, 100, 0, 0);
#endif
   // input_set_abs_params(ts->input_dev, ABS_MT_WIDTH_MAJOR, 0, 100, 0, 0);
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	char cfg_name[18];
    printk("gt9xxx parse cfg-group :%d !!!\n",sid);

    if(sid == 0){
        prop = of_find_property(np, "goodix,cfg-group0", cfg_len);
    ...
    if(sid == 1){
		prop = of_find_property(np, "goodix,cfg-group1", cfg_len);
		if (!prop || !prop->value || *cfg_len == 0 || *cfg_len > GTP_CONFIG_MAX_LENGTH) {
			return -1;/* failed */
		} else {
			memcpy(cfg, prop->value, *cfg_len);
	...
	if(sid == 2){
		prop = of_find_property(np, "goodix,cfg-group2", cfg_len);
		if (!prop || !prop->value || *cfg_len == 0 || *cfg_len > GTP_CONFIG_MAX_LENGTH) {
			return -1;/* failed */
		} else {
			memcpy(cfg, prop->value, *cfg_len);
	...
	printk("gt9xxx parse cfg-group error !!!\n");
    return -1;
...
//    GTP_INFO("GTP Driver Built@%s, %s", __TIME__, __DATE__);
    ...
    ret = gtp_i2c_test_my(client);
    if (ret < 0)
    {
        GTP_ERROR("I2C communication ERROR: gtp_i2c_test_my()");
    }
    ...
    ret = gtp_i2c_read(ts->client, buf, 4 ); 
    if (FAIL == ret){
        GTP_INFO("gtp_i2c_read 3 0x814E error");
    }else{
        GTP_INFO("gtp_i2c_read 3 0x814E value = 0x%X 0x%X",buf[2],buf[3]);
    }
