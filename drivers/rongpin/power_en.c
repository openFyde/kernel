...
struct power_en_gpio {
    ...
    struct gpio_gpio hdmi_in_en;
    struct gpio_gpio hdmi_in_rst;
	struct gpio_gpio uart5_en;
	struct gpio_gpio fan_en;
	struct gpio_gpio vddio_en;
    struct gpio_gpio baiban_hdmi_open;
	int sleep_flag;
...
static int power_en_probe(struct platform_device *pdev)
{
	...
	//------------------------------------------
        ...
        data->hdmi_in_en.gpio_num = of_get_named_gpio_flags(np, "hdmi_in_en", 0, NULL);
        if (!gpio_is_valid(data->hdmi_in_en.gpio_num))
                data->hdmi_in_en.gpio_num = -1;

        data->hdmi_in_rst.gpio_num = of_get_named_gpio_flags(np, "hdmi_in_rst", 0, NULL);
        if (!gpio_is_valid(data->hdmi_in_rst.gpio_num))
                data->hdmi_in_rst.gpio_num = -1;
        ...
        data->baiban_hdmi_open.gpio_num = of_get_named_gpio_flags(np, "baiban_hdmi_open", 0, NULL);
        if (!gpio_is_valid(data->baiban_hdmi_open.gpio_num))
                data->baiban_hdmi_open.gpio_num = -1;
...
//        	        return ret;
	...
	//yinluo add
    if(data->hdmi_in_en.gpio_num != -1){
		ret = gpio_request(data->hdmi_in_en.gpio_num, "hdmi_in_en");
        if (ret < 0){
			printk("data->hdmi_in_en request error\n");
//        	        return ret;
		}else{
			gpio_direction_output(data->hdmi_in_en.gpio_num, 1);
  			gpio_set_value(data->hdmi_in_en.gpio_num, 1);
		}
	}

    //yinluo add
    if(data->hdmi_in_rst.gpio_num != -1){
		ret = gpio_request(data->hdmi_in_rst.gpio_num, "hdmi_in_rst");
        if (ret < 0){
			printk("data->hdmi_in_rst request error\n");
//        	        return ret;
		}else{
			gpio_direction_output(data->hdmi_in_rst.gpio_num, 0);
  			gpio_set_value(data->hdmi_in_rst.gpio_num, 0);
			mdelay(100);
            gpio_direction_output(data->hdmi_in_rst.gpio_num, 1);
  			gpio_set_value(data->hdmi_in_rst.gpio_num, 1);
		}
	}

    //yinluo add
    if(data->baiban_hdmi_open.gpio_num != -1){
		ret = gpio_request(data->baiban_hdmi_open.gpio_num, "baiban_hdmi_open");
        if (ret < 0){
			printk("data->baiban_hdmi_open request error\n");
//        	        return ret;
		}else{
			gpio_direction_output(data->baiban_hdmi_open.gpio_num, 1);
  			gpio_set_value(data->baiban_hdmi_open.gpio_num, 1);
		}
	}
